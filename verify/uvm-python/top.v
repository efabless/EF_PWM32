...
module top();
    ...
    `ifdef BUS_TYPE_AHB
        ...
        wire [31:0]	HRDATA;
        wire 		HREADY;
        EF_PWM32_AHBL dut(.pwmA(pwmA), .pwmB(pwmB), .HCLK(CLK), .HRESETn(RESETn), .HADDR(HADDR), .HWRITE(HWRITE), .HSEL(HSEL), .HTRANS(HTRANS), .HWDATA(HWDATA), .HRDATA(HRDATA), .HREADY(HREADY),.HREADYOUT(HREADYOUT), .IRQ(irq));
    `endif // BUS_TYPE_AHB
