...
,
 ...
 // USE_POWER_PINS
	...
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) CMPA_REG <= 0;
                                        else if(apb_we & (PADDR[16-1:0]==CMPA_REG_OFFSET))
                                            CMPA_REG <= PWDATA[32-1:0];

	reg [31:0]	CMPB_REG;
	assign	cmpB = CMPB_REG;
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) CMPB_REG <= 0;
                                        else if(apb_we & (PADDR[16-1:0]==CMPB_REG_OFFSET))
                                            CMPB_REG <= PWDATA[32-1:0];

	reg [31:0]	TOP_REG;
	assign	top = TOP_REG;
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) TOP_REG <= 0;
                                        else if(apb_we & (PADDR[16-1:0]==TOP_REG_OFFSET))
                                            TOP_REG <= PWDATA[32-1:0];

	reg [3:0]	CLKDIV_REG;
	assign	clkdiv = CLKDIV_REG;
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) CLKDIV_REG <= 0;
                                        else if(apb_we & (PADDR[16-1:0]==CLKDIV_REG_OFFSET))
                                            CLKDIV_REG <= PWDATA[4-1:0];
	...
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) GENA_REG <= 0;
                                        else if(apb_we & (PADDR[16-1:0]==GENA_REG_OFFSET))
                                            GENA_REG <= PWDATA[12-1:0];
	...
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) GENB_REG <= 0;
                                        else if(apb_we & (PADDR[16-1:0]==GENB_REG_OFFSET))
                                            GENB_REG <= PWDATA[12-1:0];
	...
	assign	PRDATA = 
			(PADDR[16-1:0] == CMPA_REG_OFFSET)	? CMPA_REG :
			(PADDR[16-1:0] == CMPB_REG_OFFSET)	? CMPB_REG :
			(PADDR[16-1:0] == TOP_REG_OFFSET)	? TOP_REG :
			(PADDR[16-1:0] == CLKDIV_REG_OFFSET)	? CLKDIV_REG :
			(PADDR[16-1:0] == CONTROL_REG_OFFSET)	? CONTROL_REG :
			(PADDR[16-1:0] == GENA_REG_OFFSET)	? GENA_REG :
			(PADDR[16-1:0] == GENB_REG_OFFSET)	? GENB_REG :
